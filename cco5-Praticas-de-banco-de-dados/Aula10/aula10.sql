...
DELIMITER //
CREATE TRIGGER TG_INSERT_LOGCLIENTE AFTER INSERT ON CLIENTE
FOR EACH ROW
BEGIN
  DECLARE TEXTO VARCHAR(300);
    DECLARE CODIGO INT;
    
    SELECT codCli INTO CODIGO
    FROM CLIENTE
    WHERE CLIENTE.codCli = NEW.codCli;
    
    SELECT CONCAT(nomeCli, ';', Endereco, ';', cidade, ';', uf, ';', cgc, ';', cep, ';', ie, ';') INTO TEXTO
    FROM CLIENTE
    WHERE CLIENTE.codCli = NEW.codCli;
    
    INSERT INTO LOGCLIENTE(CODCLI, CONTEUDO, OPERACAO, DATAHORA) VALUES (CODIGO, TEXTO, 'INSERT', NOW());
END //
DELIMITER ;

DELIMITER //
CREATE TRIGGER TG_DELETE_LOGCLIENTE AFTER DELETE ON CLIENTE
FOR EACH ROW
BEGIN
  DECLARE TEXTO VARCHAR(300);
    DECLARE CODIGO INT;
    ...
    SELECT CODCLI INTO CODIGO
    FROM CLIENTE
    WHERE CLIENTE.CODCLI = OLD.CODCLI;
    
    SELECT CONCAT(NOMECLI, "; ", ENDERECO, "; ", CIDADE, "; ", UF, "; ", CGC, "; ", CEP, "; ", IE, ";") INTO TEXTO
    FROM CLIENTE
    WHERE CLIENTE.CODCLI = OLD.CODCLI;
    
    INSERT INTO LOGCLIENTE(CODCLI, CONTEUDO, OPERACAO, DATAHORA) VALUES (CODIGO, TEXTO, 'DELETE', NOW());
END //
DELIMITER ;

DELIMITER //
CREATE TRIGGER TG_UPDATE_LOGCLIENTE AFTER UPDATE ON CLIENTE
FOR EACH ROW
BEGIN
  DECLARE TEXTOANTIGO VARCHAR(300);
    DECLARE CODIGOANTIGO INT;
  DECLARE TEXTONOVO VARCHAR(300);
    DECLARE CODIGONOVO INT;
  
  SELECT CODCLI INTO CODIGONOVO
    FROM CLIENTE
    WHERE CLIENTE.CODCLI = NEW.CODCLI;
    
  SELECT CODCLI INTO CODIGOANTIGO
    FROM CLIENTE
    WHERE CLIENTE.CODCLI = OLD.CODCLI;
    
    SELECT CONCAT(NOMECLI, "; ", ENDERECO, "; ", CIDADE, "; ", UF, "; ", CGC, "; ", CEP, "; ", IE, ";") INTO TEXTONOVO
    FROM CLIENTE
    WHERE CLIENTE.CODCLI = NEW.CODCLI;
    
  SELECT CONCAT(NOMECLI, "; ", ENDERECO, "; ", CIDADE, "; ", UF, "; ", CGC, "; ", CEP, "; ", IE, ";") INTO TEXTOANTIGO
    FROM CLIENTE
    WHERE CLIENTE.CODCLI = OLD.CODCLI;
    
    INSERT INTO LOGCLIENTE(CODCLI, CONTEUDO, OPERACAO, DATAHORA) VALUES (CODIGONOVO, TEXTONOVO, 'UPDATE', NOW());
  INSERT INTO LOGCLIENTE(CODCLI, CONTEUDO, OPERACAO, DATAHORA) VALUES (CODIGOANTIGO, TEXTOANTIGO, 'UPDATE', NOW());
END //
DELIMITER ;
